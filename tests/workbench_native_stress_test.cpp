...
int main(int argc, char* argv[]) {
    ...
    QTimer::singleShot(
        ...
        [&] {
            ...
            EXPECT(
                editor_surface->testAttribute(
                    Qt::WA_NativeWindow));
            EXPECT(editor_surface->winId() != 0);
            EXPECT(viewport->parentWidget() == editor_surface);
            EXPECT(view_cube->parentWidget() == editor_surface);
            EXPECT(
