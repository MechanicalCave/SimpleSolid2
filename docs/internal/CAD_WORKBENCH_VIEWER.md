...
Because the production Qt/OCCT viewport is a native `WA_PaintOnScreen` child, ordinary non-native QWidget overlays are not a reliable composition mechanism on Windows. WB-01B keeps ViewCube UI-owned but makes the Editor Surface, OCCT viewport and ViewCube native-window participants; ViewCube and viewport are sibling native children under the native Editor Surface host. The existing coalesced underlay refresh remains a repaint aid, not proof that stale pixels cannot occur. Visual artifact freedom is verified manually on the exact Windows build because layout/repaint-count tests cannot prove framebuffer correctness.
